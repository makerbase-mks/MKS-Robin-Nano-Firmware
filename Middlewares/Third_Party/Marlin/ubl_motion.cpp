    ...
    void unified_bed_leveling::line_to_destination_cartesian(const float &feed_rate, const uint8_t extruder) {
      ...
      if (cell_start_xi == cell_dest_xi && cell_start_yi == cell_dest_yi) { // if the whole move is within the same cell,
        ...
        if(MACHINETPYE & IS_KINEMATIC)
            xratio = (end[X_AXIS] - mesh_index_to_xpos(cell_dest_xi)) * (1.0 / (MESH_X_DIST_IS_KINEMATIC));
        else
            xratio = (end[X_AXIS] - mesh_index_to_xpos(cell_dest_xi)) * (1.0 / (MESH_X_DIST_IS_Cartesian));
        ...
        float yratio;
        if(MACHINETPYE & IS_KINEMATIC)
            yratio = (end[Y_AXIS] - mesh_index_to_ypos(cell_dest_yi)) * (1.0 / (MESH_Y_DIST_IS_KINEMATIC));
        else
            yratio = (end[Y_AXIS] - mesh_index_to_ypos(cell_dest_yi)) * (1.0 / (MESH_Y_DIST_IS_Cartesian));
...
//  #else // UBL_SEGMENTED
#if 1
    #if 1//IS_SCARA // scale the feed rate from mm/s to degrees/s
    ...
    //#if IS_SCARA
      #define DELTA_SEGMENT_MIN_LENGTH_IS_SCARA 0.25 // SCARA minimum segment size is 0.25mm
    //#elif ENABLED(DELTA)
      #define DELTA_SEGMENT_MIN_LENGTH_IS_DELTA 0.10 // mm (still subject to DELTA_SEGMENTS_PER_SECOND)
    //#else // CARTESIAN
      //#ifdef LEVELED_SEGMENT_LENGTH
        #define DELTA_SEGMENT_MIN_LENGTH_SEG_LENGTH LEVELED_SEGMENT_LENGTH
      //#else
        #define DELTA_SEGMENT_MIN_LENGTH_IS_CARTESIAN 1.00 // mm (similar to G2/G3 arc segmentation)
      //#endif
    //#endif
    ...
    bool /*_O2*/ unified_bed_leveling::prepare_segmented_line_to(const float (&rtarget)[XYZE], const float &feedrate) {
      ...
      if(MACHINETPYE & IS_KINEMATIC)
      if (!position_is_reachable_IS_KINEMATIC(rtarget[X_AXIS], rtarget[Y_AXIS]))  // fail if moving outside reachable boundary
        return true; // did not move, so current_position still accurate
      else
      if (!position_is_reachable_IS_CARTESIAN(rtarget[X_AXIS], rtarget[Y_AXIS]))  // fail if moving outside reachable boundary
        return true; // did not move, so current_position still accurate
      ...
      float seconds;
      uint16_t segments,seglimit;

      //#if IS_KINEMATIC
      if(MACHINETPYE & IS_SCARA)
        {
        seconds = cartesian_xy_mm / feedrate;                                  // seconds to move xy distance at requested rate
        segments = lroundf(delta_segments_per_second * seconds);                  // preferred number of segments for distance @ feedrate
        seglimit = lroundf(cartesian_xy_mm * (1.0 / (DELTA_SEGMENT_MIN_LENGTH_IS_SCARA))); // number of segments at minimum segment length
      ...
      else if(MACHINETPYE & DELTA)
        {
          seconds = cartesian_xy_mm / feedrate;                                  // seconds to move xy distance at requested rate
          segments = lroundf(delta_segments_per_second * seconds);                 // preferred number of segments for distance @ feedrate
          seglimit = lroundf(cartesian_xy_mm * (1.0 / (DELTA_SEGMENT_MIN_LENGTH_IS_DELTA))); // number of segments at minimum segment length
          NOMORE(segments, seglimit);                                                        // limit to minimum segment length (fewer segments)
      ...
      {
        #ifdef LEVELED_SEGMENT_LENGTH
        segments = lroundf(cartesian_xy_mm * (1.0 / (DELTA_SEGMENT_MIN_LENGTH_SEG_LENGTH))); // cartesian fixed segment length
        #else
        segments = lroundf(cartesian_xy_mm * (1.0 / (DELTA_SEGMENT_MIN_LENGTH_IS_CARTESIAN)));
        #endif
      }
      //#endif
      ...
      for(;;) {  // for each mesh cell encountered during the move
        ...
        int8_t cell_xi,cell_yi;
        if(MACHINETPYE & IS_KINEMATIC)
        {
           cell_xi = (raw[X_AXIS] - (MESH_MIN_X_IS_KINEMATIC)) * (1.0 / (MESH_X_DIST_IS_KINEMATIC)); 
           cell_yi = (raw[Y_AXIS] - (MESH_MIN_Y_IS_KINEMATIC)) * (1.0 / (MESH_X_DIST_IS_KINEMATIC));
        }
        else
        {
           cell_xi = (raw[X_AXIS] - (MESH_MIN_X_IS_Cartesian)) * (1.0 / (MESH_X_DIST_IS_Cartesian)); 
           cell_yi = (raw[Y_AXIS] - (MESH_MIN_Y_IS_Cartesian)) * (1.0 / (MESH_X_DIST_IS_Cartesian));            
        }
        ...
        if(MACHINETPYE & IS_KINEMATIC)
        {
            z_xmy0 = (z_x1y0 - z_x0y0) * (1.0 / (MESH_X_DIST_IS_KINEMATIC));   // z slope per x along y0 (lower left to lower right)
            z_xmy1 = (z_x1y1 - z_x0y1) * (1.0 / (MESH_X_DIST_IS_KINEMATIC));   // z slope per x along y1 (upper left to upper right)
        }
        else
        {
            z_xmy0 = (z_x1y0 - z_x0y0) * (1.0 / (MESH_X_DIST_IS_Cartesian));   // z slope per x along y0 (lower left to lower right)
            z_xmy1 = (z_x1y1 - z_x0y1) * (1.0 / (MESH_X_DIST_IS_Cartesian));   // z slope per x along y1 (upper left to upper right)            
        }
        ...
        const float z_cxy1 = z_x0y1 + z_xmy1 * cx,            // z height along y1 at cx
              ...
              float z_cxym ;
          if(MACHINETPYE & IS_KINEMATIC)    
              z_cxym= z_cxyd * (1.0 / (MESH_Y_DIST_IS_KINEMATIC));  // z slope per y along cx from y0 to y1 (changes for each cx in cell)
          else
              z_cxym= z_cxyd * (1.0 / (MESH_Y_DIST_IS_Cartesian));
        ...
        const float z_sxy0 = z_xmy0 * diff[X_AXIS];                                    // per-segment adjustment to z_cxy0
              float z_sxym;  
        if(MACHINETPYE & IS_KINEMATIC)      
        z_sxym = (z_xmy1 - z_xmy0) * (1.0 / (MESH_Y_DIST_IS_KINEMATIC)) * diff[X_AXIS];  // per-segment adjustment to z_cxym
        else
        z_sxym = (z_xmy1 - z_xmy0) * (1.0 / (MESH_Y_DIST_IS_Cartesian)) * diff[X_AXIS]; 
        ...
        for(;;) {  // for all segments within this mesh cell
          ...
          if(MACHINETPYE & IS_KINEMATIC)
          {
            if (!WITHIN(cx, 0, MESH_X_DIST_IS_KINEMATIC) || !WITHIN(cy, 0, MESH_Y_DIST_IS_KINEMATIC))    // done within this cell, break to next
                break;
          }
          else
          {
            if (!WITHIN(cx, 0, MESH_X_DIST_IS_Cartesian) || !WITHIN(cy, 0, MESH_Y_DIST_IS_Cartesian))    // done within this cell, break to next
                break;
          }           
