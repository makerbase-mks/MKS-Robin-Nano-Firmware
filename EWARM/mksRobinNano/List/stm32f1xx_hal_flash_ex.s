...
//    List file    =  
//        E:\code\Robin_Nano_SVN\branches\robin_nano35源码\robin_nano35_v1.0.4_source\EWARM\mksRobinNano/List\stm32f1xx_hal_flash_ex.s
//
///////////////////////////////////////////////////////////////////////////////
...
//  242         CLEAR_BIT(FLASH->CR, FLASH_CR_MER);
        LDR.W    R0,??DataTable14_1  ;; 0x40022010
        LDR      R1,[R0, #+0]
        BIC      R1,R1,#0x4
        STR      R1,[R0, #+0]
...
??HAL_FLASHEx_Erase_IT_1:
        LDR.W    R2,??DataTable14_1  ;; 0x40022010
        LDR      R3,[R2, #+0]
        ORR      R3,R3,#0x1400
        STR      R3,[R2, #+0]
//  354 
...
//  431     SET_BIT(FLASH->CR, FLASH_CR_OPTER);
        LDR.N    R6,??DataTable14_1  ;; 0x40022010
        LDR      R0,[R6, #+0]
        ORR      R0,R0,#0x20
        STR      R0,[R6, #+0]
...
HAL_FLASHEx_OBGetUserData:
        LDR.N    R1,??DataTable14_2  ;; 0x4002201c
        LDR.N    R2,??DataTable14_3  ;; 0x1ffff804
        CMP      R0,R2
        LDR      R0,[R1, #+0]
...
//  639     SET_BIT(FLASH->CR, FLASH_CR_MER);
        LDR.N    R0,??DataTable14_1  ;; 0x40022010
        LDR      R1,[R0, #+0]
        ORR      R1,R1,#0x4
        STR      R1,[R0, #+0]
...
//  719       SET_BIT(FLASH->CR, FLASH_CR_OPTPG);
        LDR.W    R9,??DataTable14_1  ;; 0x40022010
        LDR      R1,[R9, #+0]
        ORR      R1,R1,#0x10
        STR      R1,[R9, #+0]
//  720 
...
//  724         OB->WRP0 &= WRP0_Data;
        LDR.N    R0,??DataTable14_4  ;; 0x1ffff808
        LDRH     R1,[R0, #+0]
        ANDS     R1,R4,R1
        STRH     R1,[R0, #+0]
//  725         
...
//  929       WRITE_REG(OB->RDP, ReadProtectLevel);
        LDR.N    R0,??DataTable14_5  ;; 0x1ffff800
        STRH     R4,[R0, #+0]
//  930       
...
//  971     SET_BIT(FLASH->CR, FLASH_CR_OPTPG); 
        LDR.N    R5,??DataTable14_1  ;; 0x40022010
        LDR      R0,[R5, #+0]
        ORR      R0,R0,#0x10
        STR      R0,[R5, #+0]
//  972  
...
//  976     OB->USER = (UserConfig | 0x88);
        ORR      R0,R6,#0x88
        LDR.N    R1,??DataTable14_6  ;; 0x1ffff802
        STRH     R0,[R1, #+0]
...
// 1017     SET_BIT(FLASH->CR, FLASH_CR_OPTPG); 
        LDR.N    R7,??DataTable14_1  ;; 0x40022010
        LDR      R0,[R7, #+0]
        ORR      R0,R0,#0x10
        STR      R0,[R7, #+0]
...
FLASH_OB_GetWRP:
        LDR.N    R0,??DataTable14_7  ;; 0x40022020
        LDR      R0,[R0, #+0]
        BX       LR               ;; return
...
FLASH_OB_GetUser:
        LDR.N    R0,??DataTable14_2  ;; 0x4002201c
        LDR      R0,[R0, #+0]
        UBFX     R0,R0,#+2,#+3
        BX       LR               ;; return
...
// 1119     SET_BIT(FLASH->CR, FLASH_CR_PER);
        LDR.N    R1,??DataTable14_1  ;; 0x40022010
        LDR      R2,[R1, #+0]
        ORR      R2,R2,#0x2
        STR      R2,[R1, #+0]
