...
#include "MarlinConfig.h"
  ...
  class unified_bed_leveling {
    ...
    public:
      ...
      static mesh_index_pair find_closest_mesh_point_of_type(const MeshPointType, const float&, const float&, const bool, /*uint16_t*/unsigned int[16]);
    ...
    //  static void G29(); //_O0;                          // O0 for no optimization
     	static void gcode_G29_UBL();
      static void smart_fill_wlsf(const float &); //_O2; // O2 gives smaller code than Os on A2560

			static void mesh_index_to_xypos_init();
      ...
      static float _mesh_index_to_xpos[16];
      static float _mesh_index_to_ypos[16];
      /*
      ...
      inline static float z_correction_for_x_on_horizontal_mesh_line(const float &rx0, const int x1_i, const int yi) {
        ...
        //const 
				float xratio,z1;
				if(MACHINETPYE&IS_KINEMATIC)
					xratio = (rx0 - mesh_index_to_xpos(x1_i)) * (1.0 / (MESH_X_DIST_IS_KINEMATIC));
				else
					xratio = (rx0 - mesh_index_to_xpos(x1_i)) * (1.0 / (MESH_X_DIST_IS_Cartesian));
      ...
      inline static float z_correction_for_y_on_vertical_mesh_line(const float &ry0, const int xi, const int y1_i) {
        ...
        //const 
					...
					if(MACHINETPYE == IS_KINEMATIC)
					{
						yratio = (ry0 - mesh_index_to_ypos(y1_i)) * (1.0 / (MESH_Y_DIST_IS_KINEMATIC)),
            z1 = z_values[xi][y1_i];
					}
					else
					{
						yratio = (ry0 - mesh_index_to_ypos(y1_i)) * (1.0 / (MESH_Y_DIST_IS_Cartesian)),
            z1 = z_values[xi][y1_i];					
					}
      ...
      FORCE_INLINE static float mesh_index_to_xpos(const uint8_t i) {
				if(MACHINETPYE & IS_KINEMATIC)
        	return i < GRID_MAX_POINTS_X ? /*pgm_read_float(&_mesh_index_to_xpos[i])*/_mesh_index_to_xpos[i] : MESH_MIN_X_IS_KINEMATIC+ i * (MESH_X_DIST_IS_KINEMATIC);
				else
					return i < GRID_MAX_POINTS_X ? /*pgm_read_float(&_mesh_index_to_xpos[i])*/_mesh_index_to_xpos[i] : MESH_MIN_X_IS_Cartesian+ i * (MESH_X_DIST_IS_Cartesian);
      }

      FORCE_INLINE static float mesh_index_to_ypos(const uint8_t i) {
				if(MACHINETPYE & IS_KINEMATIC)
        	return i < GRID_MAX_POINTS_Y ? /*pgm_read_float(&_mesh_index_to_ypos[i])*/_mesh_index_to_ypos[i] : MESH_MIN_Y_IS_KINEMATIC+ i * (MESH_Y_DIST_IS_KINEMATIC);
				else
					return i < GRID_MAX_POINTS_Y ? /*pgm_read_float(&_mesh_index_to_ypos[i])*/_mesh_index_to_ypos[i] : MESH_MIN_Y_IS_Cartesian+ i * (MESH_Y_DIST_IS_Cartesian);
      }
...
//  FORCE_INLINE void gcode_G29() { ubl.G29(); }
	FORCE_INLINE void gcode_G29_UBL() { ubl.gcode_G29_UBL(); }
