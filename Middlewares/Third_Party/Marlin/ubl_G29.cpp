...
//  void unified_bed_leveling::G29() {
  void unified_bed_leveling::gcode_G29_UBL() {
    ...
    if (parser.seen('Q')) {
      ...
      if (parser.seen('J')) {
        if (g29_grid_size) {  // if not 0 it is a normal n x n grid being probed
          ...
          {
            do_blocking_move_to_xy(0.5 * (MESH_MAX_X_IS_KINEMATIC- (MESH_MIN_X_IS_KINEMATIC)), 0.5 * (MESH_MAX_Y_IS_KINEMATIC - (MESH_MIN_Y_IS_KINEMATIC)));
          }
          else
          {
            do_blocking_move_to_xy(0.5 * (MESH_MAX_X_IS_Cartesian - (MESH_MIN_X_IS_Cartesian)), 0.5 * (MESH_MAX_Y_IS_Cartesian - (MESH_MIN_Y_IS_Cartesian)));
          }
          ...
          if (g29_c_flag) {
            ...
            switch((int)truncf(cvf * 10.0) - 30) {   // 3.1 -> 1
                ...
                case 1: {
                  ...
                  float weight_power  = (cvf - 3.10) * 100.0,  // 3.12345 -> 2.345
                        weight_factor = weight_power ? POW(float(10.0), weight_power) : 0;
  ...
  void unified_bed_leveling::shift_mesh_height() {
    ...
    void unified_bed_leveling::probe_entire_mesh(const float &rx, const float &ry, const bool do_ubl_mesh_map, const bool stow_probe, bool close_or_far) {
      ...
      do_blocking_move_to_xy(
        constrain(rx - (X_PROBE_OFFSET_FROM_EXTRUDER), MESH_MIN_X_IS_KINEMATIC, MESH_MAX_X_IS_KINEMATIC),
        constrain(ry - (Y_PROBE_OFFSET_FROM_EXTRUDER), MESH_MIN_Y_IS_KINEMATIC, MESH_MAX_Y_IS_KINEMATIC)
      );
      }
      else
      {
      do_blocking_move_to_xy(
        constrain(rx - (X_PROBE_OFFSET_FROM_EXTRUDER), MESH_MIN_X_IS_Cartesian, MESH_MAX_X_IS_Cartesian),
        constrain(ry - (Y_PROBE_OFFSET_FROM_EXTRUDER), MESH_MIN_Y_IS_Cartesian, MESH_MAX_Y_IS_Cartesian)
      );        
      }
    }
    ...
    float unified_bed_leveling::measure_business_card_thickness(const float &in_height) {
      ...
      if(MACHINETPYE == IS_KINEMATIC)
      {
        do_blocking_move_to(0.5 * (MESH_MAX_X_IS_KINEMATIC - (MESH_MIN_X_IS_KINEMATIC)), 0.5 * (MESH_MAX_Y_IS_KINEMATIC - (MESH_MIN_Y_IS_KINEMATIC)), in_height);
      }
      else
      {
        do_blocking_move_to(0.5 * (MESH_MAX_X_IS_Cartesian- (MESH_MIN_X_IS_Cartesian)), 0.5 * (MESH_MAX_Y_IS_Cartesian - (MESH_MIN_Y_IS_Cartesian)), in_height);
      }
  ...
  bool unified_bed_leveling::g29_parameter_parsing() {
    ...
    if(MACHINETPYE & IS_KINEMATIC)
    {
        if (!WITHIN(g29_x_pos, X_MIN_BED_IS_KINEMATIC, X_MAX_BED_IS_KINEMATIC)) g29_x_pos = X_CENTER_IS_KINEMATIC;
        if (!WITHIN(g29_y_pos, Y_MIN_BED_IS_KINEMATIC, Y_MAX_BED_IS_KINEMATIC)) g29_y_pos = Y_CENTER_IS_KINEMATIC;
    }
    else
    {
        if (!WITHIN(g29_x_pos, X_MIN_BED_IS_CARTESIAN, X_MAX_BED_IS_CARTESIAN)) g29_x_pos = X_CENTER_IS_CARTESIAN;
        if (!WITHIN(g29_y_pos, Y_MIN_BED_IS_CARTESIAN, Y_MAX_BED_IS_CARTESIAN)) g29_y_pos = Y_CENTER_IS_CARTESIAN;        
    }
  ...
  void unified_bed_leveling::g29_what_command() {
    ...
    if(MACHINETPYE & IS_KINEMATIC)
    {
        SERIAL_ECHOLNPAIR("MESH_MIN_X  " STRINGIFY(MESH_MIN_X_IS_KINEMATIC) "=", MESH_MIN_X_IS_KINEMATIC);
        SERIAL_ECHOLNPAIR("MESH_MIN_Y  " STRINGIFY(MESH_MIN_Y_IS_KINEMATIC) "=", MESH_MIN_Y_IS_KINEMATIC);
        safe_delay(25);
        SERIAL_ECHOLNPAIR("MESH_MAX_X  " STRINGIFY(MESH_MAX_X_IS_KINEMATIC) "=", MESH_MAX_X_IS_KINEMATIC);
        SERIAL_ECHOLNPAIR("MESH_MAX_Y  " STRINGIFY(MESH_MAX_Y_IS_KINEMATIC) "=", MESH_MAX_Y_IS_KINEMATIC);
    }
    else
    {
        SERIAL_ECHOLNPAIR("MESH_MIN_X  " STRINGIFY(MESH_MIN_X_IS_Cartesian) "=", MESH_MIN_X_IS_Cartesian);
        SERIAL_ECHOLNPAIR("MESH_MIN_Y  " STRINGIFY(MESH_MIN_Y_IS_Cartesian) "=", MESH_MIN_Y_IS_Cartesian);
        safe_delay(25);
        SERIAL_ECHOLNPAIR("MESH_MAX_X  " STRINGIFY(MESH_MAX_X_IS_Cartesian) "=", MESH_MAX_X_IS_Cartesian);
        SERIAL_ECHOLNPAIR("MESH_MAX_Y  " STRINGIFY(MESH_MAX_Y_IS_Cartesian) "=", MESH_MAX_Y_IS_Cartesian);        
    }
    safe_delay(25);
    ...
    if(MACHINETPYE & IS_KINEMATIC)
    {
        SERIAL_ECHOLNPAIR("MESH_X_DIST  ", MESH_X_DIST_IS_KINEMATIC);
        SERIAL_ECHOLNPAIR("MESH_Y_DIST  ", MESH_Y_DIST_IS_KINEMATIC);
    }
    else
    {
        SERIAL_ECHOLNPAIR("MESH_X_DIST  ", MESH_X_DIST_IS_Cartesian);
        SERIAL_ECHOLNPAIR("MESH_Y_DIST  ", MESH_Y_DIST_IS_Cartesian);
    }
    ...
    for (int8_t i = 0; i < GRID_MAX_POINTS_X; i++) {
      for (int8_t j = 0; j < GRID_MAX_POINTS_Y; j++) {
        ...
        if (isnan(z_values[i][j])) { // Check to see if this location holds an invalid mesh point
          ...
          if(MACHINETPYE & IS_KINEMATIC)
          {
            if (!position_is_reachable_by_probe_IS_KINEMATIC(mx, my))  // make sure the probe can get to the mesh point
                continue;
          }
          else
          {
              if (!position_is_reachable_by_probe_IS_CARTESIAN(mx, my))  // make sure the probe can get to the mesh point
                  continue;
          ...
          // from a probed location).
                ...
                d1 = (float)(HYPOT((double)(i - k), (double)(j - l)) + (1.0 / ((millis() % 47) + 13)));
  ...
  mesh_index_pair unified_bed_leveling::find_closest_mesh_point_of_type(const MeshPointType type, const float &rx, const float &ry, const bool probe_as_reference, /*uint16_t*/unsigned int bits[16]) {
    mesh_index_pair out_mesh;
    out_mesh.x_index = out_mesh.y_index = -1;
    ...
    for (int8_t i = 0; i < GRID_MAX_POINTS_X; i++) {
      for (int8_t j = 0; j < GRID_MAX_POINTS_Y; j++) {
        ...
        if ( (type == INVALID && isnan(z_values[i][j]))  // Check to see if this location holds the right thing
          ...
          if(MACHINETPYE & IS_KINEMATIC)
          {
            if (probe_as_reference ? !position_is_reachable_by_probe_IS_KINEMATIC(mx, my) : !position_is_reachable_IS_KINEMATIC(mx, my))
                continue;
          }
          else
          {
            if (probe_as_reference ? !position_is_reachable_by_probe_IS_CARTESIAN(mx, my) : !position_is_reachable_IS_CARTESIAN(mx, my))
                continue;    
           }
...
*/
    static const smart_fill_info info[]=
    {
    ...
    for (uint8_t i = 0; i < COUNT(info); ++i) {
      const smart_fill_info *f = &info[i];//(smart_fill_info*)pgm_read_ptr(&info[i]);
...
*/
      ...
      if(MACHINETPYE == IS_KINEMATIC)
      {
            x_min = max(MIN_PROBE_X, MESH_MIN_X_IS_KINEMATIC),
            x_max = min(MAX_PROBE_X, MESH_MAX_X_IS_KINEMATIC),
            y_min = max(MIN_PROBE_Y, MESH_MIN_Y_IS_KINEMATIC),
            y_max = min(MAX_PROBE_Y, MESH_MAX_Y_IS_KINEMATIC);
      }
      else
      {
            x_min = max(MIN_PROBE_X, MESH_MIN_X_IS_Cartesian),
            x_max = min(MAX_PROBE_X, MESH_MAX_X_IS_Cartesian),
            y_min = max(MIN_PROBE_Y, MESH_MIN_Y_IS_Cartesian),
            y_max = min(MAX_PROBE_Y, MESH_MAX_Y_IS_Cartesian);      
      }
    ...
    void unified_bed_leveling::smart_fill_wlsf(const float &weight_factor) {
      ...
      uint16_t bitmap[GRID_MAX_POINTS_X_MAX] = { 0 };
      struct linear_fit_data lsf_results;
      ...
      if(MACHINETPYE & IS_KINEMATIC)
      {
        weight_scaled = weight_factor * max(MESH_X_DIST_IS_KINEMATIC, MESH_Y_DIST_IS_KINEMATIC);
      }
      else
      {
        weight_scaled = weight_factor * max(MESH_X_DIST_IS_Cartesian, MESH_Y_DIST_IS_Cartesian);
      }
