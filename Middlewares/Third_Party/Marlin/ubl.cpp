...
#include "math.h"
  ...
  //constexpr 
  float unified_bed_leveling::_mesh_index_to_xpos[16],
  ...
  void unified_bed_leveling::display_map(const int map_type) {
    ...
    //const 
        float current_xi,current_yi;
        if(MACHINETPYE & IS_KINEMATIC)
        {
            current_xi = get_cell_index_x(current_position[X_AXIS] + (MESH_X_DIST_IS_KINEMATIC) / 2.0);
            current_yi = get_cell_index_y(current_position[Y_AXIS] + (MESH_Y_DIST_IS_KINEMATIC) / 2.0);
        }
        else
        {
            current_xi = get_cell_index_x(current_position[X_AXIS] + (MESH_X_DIST_IS_Cartesian) / 2.0);
            current_yi = get_cell_index_y(current_position[Y_AXIS] + (MESH_Y_DIST_IS_Cartesian) / 2.0);
        }           
  ...
  void unified_bed_leveling::mesh_index_to_xypos_init() {
	  ...
	  for(i=0;i<16;i++)
	  {
	      if(MACHINETPYE & IS_KINEMATIC)
          {   
		    _mesh_index_to_xpos[i] = /*UBL_*/MESH_MIN_X +	  i * (MESH_X_DIST_IS_KINEMATIC);
		    _mesh_index_to_ypos[i] = /*UBL_*/MESH_MIN_Y +	  i * (MESH_Y_DIST_IS_KINEMATIC);
          }
          else
          {   
            _mesh_index_to_xpos[i] = /*UBL_*/MESH_MIN_X +      i * (MESH_X_DIST_IS_Cartesian);
            _mesh_index_to_ypos[i] = /*UBL_*/MESH_MIN_Y +      i * (MESH_Y_DIST_IS_Cartesian);
          }
